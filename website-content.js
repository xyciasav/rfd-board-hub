...
const publicContent=value=>({...value,events:(value.events||[]).map(event=>({...event,dateValue:event.date,date:/^\d{4}-\d{2}-\d{2}$/.test(event.date)?`${event.date}T12:00:00`:event.date}))});
...
export function createWebsiteContent({db,save,audit,dataDir}){
  ...
  async function publicRoute(req,res,url){
    if(url==='/api/public/website-content'&&req.method==='GET')return reply(res,200,{publishedAt:db.websiteContent.publishedAt||null,content:publicContent(db.websiteContent.published)},{'access-control-allow-origin':'*','cache-control':'public, max-age=60'});
