...
function applySemanticColors(){
  const momentum=document.querySelector('.insight-stories>div:first-child');
  if(momentum?.textContent.includes('gaining momentum'))momentum.classList.add('positive');
  document.querySelectorAll('.move-row').forEach(row=>{
    const change=row.querySelector('strong');
    if(change?.classList.contains('up'))row.classList.add('up');
    else if(change?.classList.contains('down'))row.classList.add('down');
...
async function installAutomaticGoals(data,api){
  ...
  const render=()=>{holder.innerHTML=`<p class="auto-goal-note">${Object.values(saved).some(Number)?'Using saved 30-day goals.':'Automatic goals are 10% above the current 30-day pace.'}</p>`+Object.keys(labels).map(key=>{const target=targets[key]*days/30,progress=target?values[key]/target*100:0,cls=progress>=100?'met':progress>=75?'close':'behind';return`<div class="goal ${cls}"><span><b>${labels[key]}</b><small>${fmt(values[key])} of ${fmt(target)}</small></span><strong>${progress.toFixed(0)}%</strong><i><em style="width:${Math.min(progress,100)}%"></em></i></div>`}).join('')};
  render();
